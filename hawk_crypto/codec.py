...
def decodeint(x):
    ...
    for i in range(len(x)):
        ...
        # int() keeps c a Python int: NumPy >= 2 would otherwise promote c to the
        # bit array's uint8 dtype and overflow on the first value above 255.
        c += int(x[len(x) - 1 - i])
    return c
